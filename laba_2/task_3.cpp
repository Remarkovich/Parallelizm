...
std::vector<double> coefficients_matrix;  // матрица коэффициентов
std::vector<double> solutions_vector;  // вектор ответов на уравнения
...
void check_accuracy(std::vector<double>& x, std::vector<double>& x_predict, int numThreads, double sum_of_residuals, double sum_of_squared_residuals)
{
    int is_switched = 0;
...
std::vector<double> matrix_vector_product_omp(std::vector<double>& x, int numThreads, double sum_of_squared_residuals)
{
    std::vector<double> x_predict(width);
...
std::vector<double> matrix_vector_product_omp_second(std::vector<double>& x, int numThreads, double sum_of_squared_residuals)
{
    std::vector<double> x_predict(width);
    double sum_of_residuals = 0;
#pragma omp parallel num_threads(numThreads)
...
void run_parallel(int numThreads)
{
    ...
    coefficients_matrix.resize(height * width);
    solutions_vector.resize(height);
